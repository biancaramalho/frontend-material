...
![exemplo de elemento HTML](./screenshot/elemento-html.png) 
...
- **Tag de fechamento**: É a mesma que a tag de abertura, exceto que inclui uma barra antes do nome do elemento (</>). Isso indica onde o elemento termina. Pelo amor de Odin, criatura, se você esquece de adicionar essa tag de fechamento (é um erro bem comum no início, aliás) pode levar a um erro de syntax.
- **Conteúdo**: O nome já diz, certo? É o conteúdo do elemento, que, neste caso, está vazio. Mas se você voltar lá na imagem do parágrafo ```(<p>)```, voce perceberá que existe apenas um texto: "Meu gato é um deus egípcio" - este é o conteúdo da sua tag.
- **Elemento**: A tag de abertura, a tag de fechamento e o conteúdo juntos constituem o elemento, simples assim.

Certo, então voltando ao exmeplo do livro, dentro da capa existem páginas que iniciam e terminam, correto? E Normalmente não temos apenas uma página, temos várias delas.
...
- ```<!DOCTYPE html>```, É obrigatório. Uma breve história sobre, lá por volta de 1991/92, quando o HTML era apenas um bebê, os doctypes deveriam atuar como links (conexões) para um conjunto de regras que essas páginas tinham que seguir para ser consideradas um bom HTML, mas hoje são basicamente necessários para garantir que seu documento se comporte corretamente. 

Em seguida, para o navegador poder ler e interpretar os elementos do html, precisamos colocar as tags:
- ```<html></html>```, essa tag age como a "raíz" do nosso documento. Eu sei o que está pensando e já lhe respondo que sim, podemos escrever códigos CSS e JavaScript dentro do próprio documento HTML, mas já adianto que passamos de 2005 há muito tempo e hoje temos recursos que melhoram a leitura do seu código, pois fazer isso não é uma boa prática de desenvolvimento. Estas precisam ser escritas em documentos separados e importados nas metadatas e script do documento HTML. Não se preocupe, especificarei sobre isso depois. 

Agora presta atenção aqui, depois temos a tag:
- ```<head></head>```, onde nada será exibido para o usuário. Então para que serve? Para você colocar atributos do seu documento, né criatura, como as metadatas, scripts, descrição do site, é também onde colocamos link SEO e de importação de sites terceiros ou até mesmo arquivos internos e externos. É como se fosse mais um "cérebro" do que "cabeça" (tradução livre haha). 

Em seguida é muito importante lembrar da tag:
- ```<title></title>``` a qual especificamos o título do site, mas não é o título que aparecerá no layout não, e sim o que estará escrito lá na aba de navegação que, se você for igual eu vai abrindo abas como se não houvesse amanhã, no exemplo abaixo preste atenção apenas no que representam abas: 

![exemplo de abas do navegador](./screenshot/abas.png) 

Em seguida, inserimos a tag:
- ```<body></body>``` que é onde toda a estrutura que você construiu para o site irá aparecer para o usuário. Então será ali que inserimos as tags de título, imagens, animações, parágrafos e etc. Abaixo, temos um exemplo de como fica a estrutura inicial do documento HTML: 
...
Não copie, criatura, trate de fazer na mão para exercitar essa memória muscular aí, to de olho hein?!

### Surfando nas tags

Aqui vou falar mais sobre a semântica de cada tag (que é muito importante, por isso não pule essa parte) que faz todo o sentindo na hora de construir seu documento HTML. Tá, mas entendendo o que diacho é semântica? Criatura, HTML é uma linguagem semântica e ela é o que ajuda os mecanismos de busca (SEO) classificar a importância e relevância das informações contidas na sua página, por exemplo. Além disso, é totalmente útil quando se trata de inclusão digital de deficientes visuais. Com este intuito, novas tags foram criadas, dando maior importância e semântica aos textos. 
...
Mas voltando as tags, elas se comportam como a clássica (e antiga, por favor) Tag - ```<div>```, que tinha este nome poquê dividia textos em blocos – até então anônimos (porque as tags não tinha uma classificação e usabilidade). Agora, eles podem ser classificados. Seguem as principais possibilidades:
...
```<header></header> e <footer></footer>``` São tags bem simples, mas assim como a "nav" têm um propósito importante. Cabeçalhos e rodapés também funcionam bem com SEO. As tags separam os itens de um cabeçalho e de um rodapé pois elas são diferentes de uma tag "div". Ao serem identificados, os mecanismos de pesquisa são capazes de diferenciar as informações de outro conteúdo e classificar seu site de acordo com o que ele propõe.

**VIDEO E ÁUDIO** 

```<video></video> e <audio></audio>``` A multimídia está se tornando cada vez mais popular entre os usuários e já se foram os dias em que os visitantes só podiam consumir informações por texto, né. Voltamos no assunto de inclusão digital aqui (sim, preciso fazer uma pauta sobre isso, prometo). As tags para vídeo e som (áudio) são fáceis de implementar em HTML5 pois utilizam propriedades que correspondem exatamente o que necessita. 
...
Sim, você pode colocar elementos dentro de outros elementos, isso é o que chamamos de *aninhamento*. As Tags de ênfase são para destacar algo em seus textos, e para utilizá-los aninhamos os elementos. Existem diversas maneiras de destacar texto, as mais usuais são ```<strong></strong>``` que deixa o texto destacado por padrão e ```<em></em>```, que deixa o texto itálico. O "strong" que traz uma semântica de força enquanto o itálico traz um sentido de intensidade, por exemplo: *"Está muuuuuito quente lá fora"*. É muito importante abrir e fechar as tags de enfase antes de fechar a tag de parágrafo para que o navegador interprete da forma correta. Ex: 

```<p>Está <strong>muito</strong> quente lá fora.</p>```

Fica assim: 

![exemplo de ênfase](./screenshot/ex-strong.png)
...
- Próprias listas, sejam elas ordenadas ou não;
- Navegações;
...
![exemplo de lista ordenada](./screenshot/ex-ordenada.png)
...
**IMPORTANTE**: As imagens não são válidas sem o atributo "alt", elas podem até funcionar sem ele, mas por questão de semântica (lembra que HTML é uma linguagem de semântica), deve ser inserido algum nome ou breve descrição para que melhore o desempenho de busca pelos mecanismos de pesquisa e também para que pessoas com deficiência visual possam saber o que estão acessando. No caso de logos ou elementos decorativos, não necessita atribuir um valor ao "alt", mas ainda é requerido por conta da semântica, mesmo que seja com um valor vazio. Exemplo: 
...
- É mais viável utilizar letras minúscula para não se enrolar com os títulos dos arquivos, mas a regra só vale para docs HTML, hein!?
...
- E por último e não menos importante, imagens, logos e demais fontes que não sejam os arquivos propriamente ditos, devem estar em alguma subpasta para melhor organização. 

### Conclusão 

É muito importante aprendermos todas as linguagens possíveis (não somente as de programação) se deseja desenvolver um projeto bacana que dê ótimos resultados. Passando brevemente pela história da HTML, ela foi inventada por **Tim Berners-Lee**, um físico do centro de pesquisas **CERN**, na Suíça. Ela surgiu com a ideia de um sistema de hipertexto na internet, quando a mesma começou a se expandir.

De acordo com o Elemento de Referência do HTML, do Mozilla Developer Network, atualmente existem 140 tags HTML, mesmo que algumas delas já estejam obsoletas (não mais suportadas pelos navegadores). Mas obviamente não decoramos tudo isso de tag, apenas as principais, se quisermos saber mais detalhes, pode consutar o site [html.com](https://html.com/).

HTML tornou-se uma linguagem muito popular e em uma velocidade absurda, agora é considerada um padrão oficial da web. As especificações do HTML são mantidas e desenvolvidas pelo World Wide Web (W3C). Mas por que estou efatizando tudo isso? Saber fazer um documento HTML hoje em dia pode parecer fácil para quem esteja começando, e mais fácil ainda para quem já tem experiência, mas o mais importante é saber o seu significado, a sua existência e também a utilidade. 

HTML é mais que semântica, com ela vem a evolução e conquista virtual de pequenos negócios, bem como ajuda na inclusão de deficientes visuais, fazendo com que sites estejam cada vez mais acessíveis para todo o tipo de público com um dispositivo móvel e acesso a internet na mão, enfim. É sobre isso sabe &#10084;.

Espero que tenham gostado, eu tentei o máximo possível ajudar quem precisa com esse texto singelo. 
